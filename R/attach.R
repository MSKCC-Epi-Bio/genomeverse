...
pkgs <- c("cbioportalR", "gnomeR", "oncokbR")

genomeverse_attach <- function() {
  # Create `to_load` which is a character vector of all genomeverse
  ...
  # genomeverse with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("genomeverse ", package_version("genomeverse"))
  )

  # Return a character string containing the package version for each of genomeverse's constituents
...
genomeverse_detach <- function() {
  pak <- paste0("package:", c(pkgs, "genomeverse"))
  lapply(pak[pak %in% search()], detach, character.only = TRUE)
  invisible()
}

#' List all packages imported by genomeverse
#'
#' @export
#'
#' @examples
#' genomeverse_packages()
genomeverse_packages <- function() {
  # get all imports from genomeverse's package description file
  raw <- utils::packageDescription("genomeverse")$Imports
